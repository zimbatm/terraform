...
func TestInterpolateFuncFormatMap(t *testing.T) {
	testFunction(t, testFunctionConfig{
		Cases: []testFunctionCase{
			{
				`${formatmap(var.list, var.list2)}`,
			...
			{
				`${formatmap(var.list, var.nonstrings)}`,
				map[string]interface{}{
			...
			{
				`${formatmap(var.nonstrings, var.list2)}`,
				nil,
				true,
			},
			{
				`${formatmap(var.list, var.differentlengthlist)}`,
				nil,
				true,
			},
		},
