...
func testAccCheckVaultTokenAttributes(
	...
	return func(s *terraform.State) error {
		...
		// Check within a few seconds since TTL counts down from secret creation
		if (expectedDuration - gotDuration) > (2 * time.Second) {
			return fmt.Errorf("Expected TTL %s, got %s",
